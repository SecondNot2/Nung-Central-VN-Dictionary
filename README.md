...
# Với production, nên đặt model tường minh thay vì dựa vào default
# ROUTER_MODEL=NungDic
#
...
Nếu gặp lỗi kiểu `No active credentials for provider: gemini-cli`, nghĩa là model hiện tại đang trỏ tới provider chưa được kết nối trong dashboard 9Router. Khi đó cần:

1. Vào 9Router Dashboard -> Providers -> Connect Gemini CLI để đăng nhập Google OAuth.
2. Hoặc đổi `ROUTER_MODEL` / `LLM_MODEL` sang model alias đang hoạt động của bạn, ví dụ `NungDic`.
